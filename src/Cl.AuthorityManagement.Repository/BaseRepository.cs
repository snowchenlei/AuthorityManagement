...
namespace Cl.AuthorityManagement.Repository
{
    public class BaseRepository<T> where T : class, new()
    {
        ...
        {
            return Db.Set<T>().Any(whereLamada);
        }
        ...
        {
            return Db.Set<T>().Where(whereLamada).Select(selectLamada).FirstOrDefault();
        }
        ...
        {
            return await Db.Set<T>().Where(whereLamada).Select(selectLamada).FirstOrDefaultAsync();
        }
        ...
        public T LoadFirst(Expression<Func<T, bool>> whereLamada, bool isAsNoTracking = false)
        {
            if (isAsNoTracking)
            {
                return Db.Set<T>().AsNoTracking().FirstOrDefault(whereLamada);
            }
            else
            {
                return Db.Set<T>().FirstOrDefault(whereLamada);
            }
        }
        ...
        public IQueryable<T> LoadEntities(Expression<Func<T, bool>> whereLamada, bool isAsNoTracking = false)
        {
            if (isAsNoTracking)
            {
                return Db.Set<T>().AsNoTracking().Where(whereLamada);
            }
            else
            {
                return Db.Set<T>().Where(whereLamada);
            }
        }
        ...
        public IQueryable<T> LoadPageEntities<s>(int pageIndex, int pageSize, out int totalCount, Expression<Func<T, bool>> whereLamada, Expression<Func<T, s>> orderbyLamada, bool isASC = true, bool isAsNoTracking = false)
        {
            ...
            if (isAsNoTracking)
            {
                temp = Db.Set<T>().AsNoTracking().Where(whereLamada);
            }
            else
            {
                temp = Db.Set<T>().Where(whereLamada);
            }
